...
│   │   ├── insights/           ← AI insights via Codex Haiku + 8 insights demo
...
│   │       ├── ai/insights     ← Codex Haiku, cache 1h
...
│   │   ├── ai.ts               ← Codex Haiku client (fetch nativo)
...
```bash
cd ~/Documents/Codex/Projects/lava-jatis/lavai-app
npm install
...
```bash
open ~/Documents/Codex/Projects/lava-jatis/lavai-landing.html
```
...
cd ~/Documents/Codex/Projects/lava-jatis
...
> **IMPORTANTE:** O git cria `.git/HEAD.lock` e `.git/index.lock` no sandbox. **Sempre** fazer `rm -f .git/HEAD.lock .git/index.lock` antes de commitar pelo terminal do usuário (não pelo Codex).
...
| **AI Insights** | Codex Haiku, 8 métricas, cache 1h, painel colapsável |
...
- [ ] **Chatbot IA no atendimento** — Codex responde dúvidas dos clientes do lava-jato
